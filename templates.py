...
function initControls(){const fs=document.getElementById('font-size-range');fs&&fs.addEventListener('input',()=>{st.fontSize=parseInt(fs.value,10);applySettings();saveSettings();});const lh=document.getElementById('line-height-range');lh&&lh.addEventListener('input',()=>{st.lineHeight=parseFloat(lh.value);applySettings();saveSettings();});document.querySelectorAll('#font-family-choices .chip').forEach(ch=>ch.addEventListener('click',()=>{st.font=ch.dataset.font;applySettings();saveSettings();}));document.querySelectorAll('#theme-choices .chip').forEach(ch=>ch.addEventListener('click',()=>{st.theme=ch.dataset.theme;applySettings();saveSettings();}));document.querySelectorAll('#tts-toggle-chips .chip').forEach(ch=>ch.addEventListener('click',()=>{if(window.TTS_SUPPORTED===false)return;st.ttsVisible=(ch.dataset.tts==='show');toggleTTSVisibility(st.ttsVisible,true);saveSettings();syncChips();if(st.ttsVisible&&DEFER_TTS_SEGMENT)window.dispatchEvent(new CustomEvent('tts-lazy-segment'));}));const panel=document.getElementById('settings-panel');const openBtn=document.getElementById('open-settings');const closeBtn=document.getElementById('close-settings');openBtn&&openBtn.addEventListener('click',()=>{panel.setAttribute('data-open','true');panel.setAttribute('aria-hidden','false');});closeBtn&&closeBtn.addEventListener('click',()=>{panel.setAttribute('data-open','false');panel.setAttribute('aria-hidden','true');});document.addEventListener('click',e=>{if(panel&&panel.getAttribute('data-open')==='true'&&!panel.contains(e.target)&&e.target!==openBtn){panel.setAttribute('data-open','false');panel.setAttribute('aria-hidden','true');}});const installBtn=document.getElementById('install-app-btn');if(installBtn){window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();installPromptEvt=e;installBtn.style.display='inline-flex';});installBtn.addEventListener('click',async()=>{if(!installPromptEvt)return;installPromptEvt.prompt();try{await installPromptEvt.userChoice;}catch(_){ }installPromptEvt=null;installBtn.style.display='none';});window.addEventListener('appinstalled',()=>installBtn.style.display='none');if(window.matchMedia('(display-mode: standalone)').matches){installBtn.style.display='none';}}}
